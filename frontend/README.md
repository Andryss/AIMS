...
React single-page application for the MIB Alien Incident Management System (AIMS). The production build is copied into the
Spring Boot backend and served under `/public` (see `npm run build:backend`).
...
For local API calls during development, run the [backend](../backend/README.md) on port 8080 (`proxy` in `package.json`
forwards to it).
...
The built bundle includes transitive dependencies of the packages above (for example `@remix-run/router` from React
Router). All **direct** production npm dependencies use permissive open-source licenses.
...
CRA pulls a large transitive tree (Babel, webpack, ESLint, etc.) used only at build and test time. Those packages are **not**
included in the static files deployed with the backend JAR.
...
GPL, AGPL, or LGPL libraries that would be distributed as part of the commercial product runtime. None are declared as
