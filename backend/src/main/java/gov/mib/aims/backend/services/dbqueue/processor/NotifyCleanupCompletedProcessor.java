...
/**
 * Уведомляет ответственного агента о завершении очистки.
 */
@Slf4j
@Component
@RequiredArgsConstructor
@DbQueueSettings(NotifyCleanupCompletedPayload.QUEUE_NAME)
public class NotifyCleanupCompletedProcessor implements DbQueueProcessor<NotifyCleanupCompletedPayload> {

    private final IncidentRepository incidentRepository;
    private final NotificationService notificationService;

    @Override
    public TaskExecutionResult execute(NotifyCleanupCompletedPayload payload) {
        long incidentId = payload.getIncidentId();
        Optional<IncidentEntity> incidentOpt = incidentRepository.findById(incidentId);
        if (incidentOpt.isEmpty()) {
            log.warn("Incident {} not found for cleanup completed notification", incidentId);
            return TaskExecutionResult.finish();
        }
        IncidentEntity incident = incidentOpt.get();
        Long responsibleUserId = incident.getResponsibleUserId();
        if (responsibleUserId == null) {
            log.warn("Incident {} has no responsible user for cleanup notification", incidentId);
            return TaskExecutionResult.finish();
        }
        String message = "По инциденту №" + incidentId + " завершена очистка";
        String relatedRef = EntityRef.format(EntityType.INCIDENT, incidentId);
        notificationService.send(responsibleUserId, message, List.of(relatedRef));
        return TaskExecutionResult.finish();
    }
}
